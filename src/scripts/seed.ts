...
import type { Category, CustomOrder, Order, Product } from "@/lib/types";
...
function r2Url(key: string): string {
  ...
  if (!base) throw new Error("R2_PUBLIC_URL is not set — see frontend/.env");
  return `${base.replace(/\/$/, "")}/${key}`;
}

// Already-uploaded to R2 by a one-time migration script when uploads moved off local disk.
...
// migrated to R2 when uploads moved off local disk storage.
